...
CREATE TABLE [dbo].[customer] (
    ...
    [phone]         VARCHAR(15)     NOT NULL, -- Báº¯t buá»™c cÃ³ SÄT
    [zalo_id]       VARCHAR(255)    NULL,     -- Dá»± phÃ²ng Ä‘Äƒng nháº­p Zalo
...
CREATE TABLE [dbo].[vai_tro] (
    ...
    [role_name] NVARCHAR(100) NOT NULL, -- TÃªn hiá»ƒn thá»‹ (VD: 'Quáº£n trá»‹ viÃªn', 'NhÃ¢n viÃªn bÃ¡n hÃ ng')
...
CREATE TABLE [dbo].[staff] (
    ...
    [role_id]       VARCHAR(20)     NOT NULL, -- Trá» vá» báº£ng vai_tro
...
CREATE TABLE [dbo].[campaign] (
    ...
    [duong_dan_slug] VARCHAR(255)   NULL UNIQUE, -- (đuôi link của chiến dịch) ví dụ: vui-he-2026
    [hinh_anh_url]  VARCHAR(500)    NULL, -- hình ảnh banner của chương trình
    [cauhinh_theme_json] NVARCHAR(MAX) NULL, --cấu hình giao diện Minigame Builder
...
-- 6. CAMPAIGN_RULE (Luáº­t CÆ¡ Báº£n)
...
-- 6.1 CAMPAIGN_RULE_PAYMENT (cộng lượt theo các phương thức thanh toán)
...
-- 6.2 CAMPAIGN_RULE_SKU (cộng lượt theo các sản phẩm SKU)
...
-- 15. SYSTEM_CONFIG (Cáº¥u hÃ¬nh há»‡ thá»‘ng, Zalo ZNS, API Keys)
...
BEGIN
    ...
    IF @luot_hien_tai IS NULL OR @luot_hien_tai <= 0
        THROW 50001, N'Khách hàng không còn ủ lượt quay!', 1;
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @ma_giai_thuong IS NULL THROW 50002, N'Mã Voucher không tn tại!', 1;
        IF @trang_thai_voucher = 1 THROW 50003, N'Mã Voucher này ã ược sử dụng!', 1;
        IF @trang_thai_voucher = 2 THROW 50004, N'Mã Voucher này ã hết hạn!', 1;
        
        --CH ĐƯỢC ĐI QU TẠI CỬA HNG Đ MUA HNG
        IF @ma_store_phat_hanh != @ma_store
            THROW 50006, N'Mã quà tặng này ch ược i tại cửa hàng mà bạn ã mua hàng!', 1;
...
-- THỦ TỤC LẤY DANH SÁCH GIẢI THƯNG HỢP L CHO 1 CỬA HNG (Lọc tn kho + Gii hạn User)
...
BEGIN
    ...
    WHERE spi.ma_store = @ma_store
      ...
      AND (spi.ton_kho > 0 OR spi.ton_kho = -1)             -- Cửa hàng phải còn hoặc không gii hạn
      AND (p.ton_kho_toan_he_thong > 0 OR p.ton_kho_toan_he_thong = -1) -- Kho tng phải còn hoặc không gii hạn
      AND (
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Kim tra dòng tn tại và lấy s lượng tn kho
        ...
        BEGIN
            RAISERROR('Không tìm thấy bản ghi tn kho cho cửa hàng và giải thưng này.', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END
        ...
        BEGIN
            RAISERROR('Tn kho ã hết. Không th i quà.', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END

        -- Cập nhật tn kho và s lượng ã phát
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            THROW 50000, N'S lượng cấp phải ln hơn 0 hoặc bằng -1 (không gii hạn)', 1;
        END
        ...
        BEGIN
            THROW 50000, N'Không tìm thấy giải thưng', 1;
        END
        ...
        BEGIN
            THROW 50000, N'Cửa hàng không thuc chiến dch của giải thưng này. Hãy chọn cửa hàng hợp l.', 1;
        END

        -- 3. Check global inventory
        IF @tonKhoToanHeThong <> -1 AND @quantity <> -1 AND @tonKhoToanHeThong < @quantity
        BEGIN
            DECLARE @errMsg NVARCHAR(100) = N'Tn kho tng không ủ. Hin ch còn ' + CAST(@tonKhoToanHeThong AS NVARCHAR(20));
            THROW 50000, @errMsg, 1;
        END
        
        IF @tonKhoToanHeThong <> -1 AND @quantity = -1
        BEGIN
            THROW 50000, N'Không th cấp phát không gii hạn cho giải thưng có gii hạn tn kho', 1;
        END
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            THROW 50000, N'Không tìm thấy dữ liu phân b.', 1;
        END

        IF @newTongLuongCap <> -1 AND @newTongLuongCap < @daPhat
        BEGIN
            DECLARE @errMsg1 NVARCHAR(100) = N'Không th giảm tng cấp xung dưi s lượng ã phát (' + CAST(@daPhat AS NVARCHAR(20)) + N').';
        ...
        BEGIN
            ...
            BEGIN
                THROW 50000, N'Không th chnh thành không gii hạn vì giải thưng này có gii hạn tn kho tng.', 1;
            END
            
            IF @tonKhoToanHeThong = -1 AND @newTongLuongCap <> -1
            BEGIN
                THROW 50000, N'Giải thưng không gii hạn phải luôn ược cấp không gii hạn (-1).', 1;
            END
            ...
            BEGIN
                DECLARE @errMsg2 NVARCHAR(100) = N'Tn kho tng không ủ  cấp thêm. Hin ch còn ' + CAST(@tonKhoToanHeThong AS NVARCHAR(20));
...
BEGIN
   IF EXISTS (SELECT 1 FROM inserted WHERE gia_tri_don_hang_toi_thieu < 0)
   BEGIN
       RAISERROR(N'Li: Giá tr ơn hàng ti thiu không ược nhỏ hơn 0!', 16, 1);
       ROLLBACK TRANSACTION;
   END
END
GO
...
BEGIN
   IF EXISTS (SELECT 1 FROM inserted WHERE so_luot_thuong < 0)
   BEGIN
       RAISERROR(N'Li: S lượt thưng theo phương thức thanh toán không ược nhỏ hơn 0!', 16, 1);
       ROLLBACK TRANSACTION;
   END
END
GO
...
BEGIN
   IF EXISTS (SELECT 1 FROM inserted WHERE so_luot_thuong < 0)
   BEGIN
       RAISERROR(N'Li: S lượt thưng theo sản phẩm SKU không ược nhỏ hơn 0!', 16, 1);
       ROLLBACK TRANSACTION;
   END
END
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Lấy danh sách các voucher chưa i thuc chiến dch ã hết hạn
        ...
        -- 1. Cng lại kho tng (ch cập nhật những giải có gii hạn kho != -1)
        UPDATE p
        ...
        -- 2. Cng lại kho cửa hàng và giảm da_phat
        UPDATE spi
        ...
        -- 3. Cập nhật trạng thái voucher thành -1 (Hết hạn/Đã thu hi)
        UPDATE v
        SET trang_thai = -1
        FROM reward_voucher v
...
-- =========================================================================
-- TNG HỢP CÁC VIEW DNG TRONG H THỐNG LUCKY DRAW (TỐI ƯU HA LC DỮ LIU)
-- =========================================================================

-- 1. View tn kho giải thưng tại các cửa hàng
-- (View này h trợ dashboard xem tiến  phát quà, tng lượng cấp, tn kho, v.v...)
...
-- 2. View chiến dch của từng cửa hàng
-- (H trợ xác nh cửa hàng nào ang chạy chiến dch nào)
...
-- 3. View danh sách Kho giải thưng
-- (H trợ tính toán xác suất tự ng, kim tra kho thực tế vs h thng)
...
-- 4. View Lch s trng thng & i qu (Ti u ha Lc)
-- (View ny  c nng cp JOIN thng vo bng Campaign  h tr filter t Backend khng qua Java Stream)
